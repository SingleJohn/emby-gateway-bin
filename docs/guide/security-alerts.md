...
Feiyue Emby Gateway 提供了强大的安全告警系统，用于监控和检测异常访问模式。本文档将详细介绍安全告警系统的配置和使用方法。
...
- **多维度分析**：支持按用户、IP、规则名、Source、决策类型等维度聚合分析
...
### 4. 拦截事件频率告警（security_events）

监控安全拦截事件（`deny` / `rate_limit`）在时间窗口内的增长情况。  
该类型基于独立的 `security_events` 表，不依赖 `request_logs`。

**配置步骤**：

1. 登录 Feiyue Emby Gateway 管理界面
2. 点击左侧菜单中的「安全告警」
3. 在「告警规则」卡片中，点击「新增规则」按钮
4. 填写规则信息：
   - **名称**：拦截事件突增检测
   - **类型**：拦截事件频率
   - **统计维度**：推荐 `规则名 (rule_name)`，也可选 `IP`、`Source`、`决策类型`
   - **统计窗口 (秒)**：3600
   - **冷却时间 (秒)**：1800
   - **通知渠道**：选择要使用的通知渠道
   - **适用 Source**：留空表示全部
...
- **告警类型**：访问频率、身份漂移、失败率、拦截事件频率
- **统计维度**：用户、IP、规则名、Source 或决策类型
- **统计窗口**：告警统计的时间范围
- **触发条件**：具体的触发原因和阈值

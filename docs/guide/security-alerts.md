...
如果遇到安全告警相关问题，请参考 [排障指南](../ops/troubleshooting.md#74-安全告警故障排查) 中的相关部分。
...
4. 在「告警规则」卡片中，添加以下三条规则：
   ...
   **规则 3：异常失败率**
   ...
   - 统计窗口 (秒)：3600
   - 冷却时间 (秒)：1800
   - 通知渠道：tg_alerts（示例通道 ID）
   - 最小请求数：50
   - 失败率阈值：0.3

5. 点击页面底部的「保存」按钮

**配置完成后**，系统会按照设定的周期检查告警规则，当检测到异常时通过配置的通知渠道发送告警信息。

## 总结

安全告警系统是 Emby Gateway 的重要安全功能，通过监控异常访问模式，可以及时发现和应对潜在的安全威胁。建议根据实际部署环境和安全需求，合理配置告警规则，确保系统安全的同时避免过多误报。

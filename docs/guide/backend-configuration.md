...
`/直链空间/电影/熊出没·熊心归来 (2016)/熊出没·熊心归来.2016.2160p.WEB-DL.H.264.DDP.5.1.mp4`

Root Folder ID 配置了 `直链空间` 的目录id “123456”

拼接模式生成的路径会从 `直链空间` 目录开始 

`/电影/熊出没·熊心归来 (2016)/熊出没·熊心归来.2016.2160p.WEB-DL.H.264.DDP.5.1.mp4`

这个路径是不能播放的，需要在 `路径映射` 功能配置规则，在前面加上 `/直链空间`
